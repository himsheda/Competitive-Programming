...
int DP(int i,int prev,int start,string &s,vector<vvi> &dp){
    // cout<<i<<" "<<prev<<" "<<start<<endl;
    if(dp[i][prev][start]!=-1) return dp[i][prev][start];
    // if(i==sz(s)) return 0;
    if(i==sz(s)-1){
        if(s[i]=='?') return prev!=start;
        else{
    ...
    if(s[i]!='?'){
        if(s[i]-'a'==prev) return 0;
        return dp[i][prev][start]=DP(i+1,s[i]-'a',start,s,dp);
    }
    int ans=0;
    for(int j=0;j<26;j++){
        if(j==prev) continue;

        ans=(ans+DP(i+1,j,start,s,dp)) % mod;
    }
    return dp[i][prev][start]=ans;
}
void solve(){
    string s;
    cin>>s;
    ...
    if(s[0]=='?'){
        if(s[n-1]!='?'){
            ans=DP(1,s[n-1]-'a',s[n-1]-'a',s,dp);
        }
        else{
            for(int i=0;i<26;i++){
                ans=(ans+DP(1,i,i,s,dp))%mod;
            }
        }
    }
    else{
        ans=DP(1,s[0]-'a',s[0]-'a',s,dp);
    }
    cout<<ans<<endl;
}
int32_t main(){
