...
cin >> n;
    ...
    // show(v);
    int ans = 0;
    vector<vector<int>>dp(n, vi(n));
    vector<vector<int>>w(n, vi(n));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            int u = 0;
            if (i - 1 >= 0 and j - 1 >= 0) u += dp[i - 1][j - 1];
            if (i - 1 >= 0 and j + 1 < n) u += dp[i - 1][j + 1];
            if (i - 1 >= 0) u += w[i - 1][j];
            if (i - 1 >= 0 and j - 1 >= 0 and i - 1 >= 0 and j + 1 < n and i - 2 >= 0) u -= dp[i - 2][j];
            char c = s[i][j];
            if (u % 2 == 1) c = '1' + '0' - c;
            if (c == '1') {
                ans++;
