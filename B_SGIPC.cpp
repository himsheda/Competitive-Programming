...
int DP(int n,int k,int mn,vector<vvi> &dp){
    
    if(n==0&&k==0) return 1;
    if(mn>n) return 0;
    if(k==0) return 0;
    if(n==0) return 0;
    if(dp[n][k][mn]!=-1) return dp[n][k][mn];
    int ans=0;
    ans=(ans+DP(n-mn,k-1,mn,dp))%mod;
    ans=(ans+DP(n,k,mn+1,dp))%mod;
    // cout<<n<<" "<<k<<" "<<mn<<" "<<ans<<endl;
    
    return dp[n][k][mn]= ans;
}
void solve(){
    int n,k;
    cin>>n>>k;
    int ans=0;
    vector<vvi> dp(n+1,vvi(k+1,vi(n+1,-1)));
    cout<<DP(n,k,1,dp)<<endl;
    ff(i,0,n+1){
        show(dp[i]);
    }
}
