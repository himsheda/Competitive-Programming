...
int DP(int i,int j,vi &arr,string &s,vvi &dp){
    if(dp[i][j]!=-1) return dp[i][j];
    if(j==sz(arr)) return dp[i][j]= 1;
    if(i==sz(s)) return dp[i][j]=0;
    int take=0;
    if(s[i]-'0'==arr[j]) take=DP(i+1,j+1,arr,s,dp);
    int not_take=DP(i+1,j,arr,s,dp);
...
void solve(){
    ...
    vi arr(n);
    take(arr);
    vvi dp(sz(s)+1,vi(sz(arr)+1,-1));
    cout<<DP(0,0,arr,s,dp)<<endl;
