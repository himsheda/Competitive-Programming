...
void solve(){
    ...
    sort(all(arr));
    ff(i,0,n-1){
        int d=arr[i+1].F-arr[i].F-1;
        if(d==0) continue; 
        if(d<=abs(arr[i+1].S-arr[i].S)){
            ans=max(ans,min(arr[i+1].S,arr[i].S)+d);
            // cout<<min(arr[i+1].S,arr[i].S)+d<<" ";
        }
        else{
            ans=max(ans,min(arr[i+1].S,arr[i].S)+abs(arr[i+1].S-arr[i].S)+(d-abs(arr[i+1].S-arr[i].S)+1)/2);
            // cout<<min(arr[i+1].S,arr[i].S)+abs(arr[i+1].S-arr[i].S)+(d-abs(arr[i+1].S-arr[i].S))/2<<" ";
        }
        // cout<<ans<<" ";
    }
    cout<<ans<<endl;
