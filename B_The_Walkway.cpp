...
void solve(){
   ...
   ff(i,0,m){
    if(i==0){
        int a=ceil((arr[i+1]-1)*1.0/d);
        int b=ceil((arr[i]-1)*1.0/d)+ceil((arr[i+1]-arr[i])*1.0/d);
        sum+=ceil((arr[i]-1)*1.0/d);
        if(a<b){
            count++;
            // sum+=a;
        }
        ...
        int a=ceil((n-arr[i-1])*1.0/d);
        int b=ceil((arr[i]-arr[i-1])*1.0/d)+(n-arr[i])/d;
        sum+=ceil((arr[i]-arr[i-1])*1.0/d);
        if(a<b){
            count++;
            // sum+=a;
        }
        ...
        int a=ceil((arr[i+1]-arr[i])*1.0/d);
        int b=ceil((arr[i]-arr[i-1])*1.0/d)+ceil((arr[i+1]-arr[i])*1.0/d);
        sum+=ceil((arr[i]-arr[i-1])*1.0/d);
        if(a<b){
            count++;
            // sum+=a;
